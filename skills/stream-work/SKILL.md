...
- **Spend governance.** Track app-level LLM spend against the charter envelope (the bindings doc names the project's spend record — e.g. call-record rows in the stream DB timestamped after stream open; the clone duplicates trunk history, so filter by time). Approaching the envelope → finish to a safe point and stop. Session/subscription tokens for the coding itself are outside the envelope unless the charter says otherwise.
