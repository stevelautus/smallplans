...
5. **Migrations, if trunk brought any** (the project bindings doc, `docs/PARALLEL-DEV-WORKFLOW.md` §3, is the spec):
   - Symptom of the known state: Django refuses with "multiple leaf nodes" — trunk's new `00NN` and the stream's first `9xxx` both descend from the old head.
   - Fix: edit the **first** `9xxx` migration's `dependencies` to point at the new trunk head, then run the project's migration command against the stream's private DB (Django tracks the applied set by name, not order, so the new trunk migration applies fine after the already-applied `9xxx`).
   - If `migrate` fails because both sides touched the same tables incompatibly: that is a real schema conflict, not tooling. Reconcile the `9xxx` content; if the private DB is wedged, rebuild it from a fresh trunk clone per the bindings and re-apply.
...
- Frequency guidance lives with the caller (/smallplans:stream-work checkpoints, the gate hook); this skill executes one sync correctly.
