...
1. **Validate and derive.**
   - Slug must match `^[a-z][a-z0-9_]*$` and stay short (≤ ~20 chars) — it becomes a Postgres DB-name suffix and a directory name. Reject and re-ask otherwise.
...
2. **Preconditions — stop with specific guidance if any fail; do not improvise around them.**
   ...
   - Database privileges suffice — the app role needs `CREATEDB` plus a clonable template; no superuser is involved (the bindings doc supplies the project's exact privilege check). A failure here means the machine is missing the one-time superuser setup described in the bindings' privilege note — stop and say so.
